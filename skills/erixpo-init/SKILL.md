...
7. Write `.erixpo/init-manifest.txt` listing every file **you created** (not files that already existed). Uninstall `--purge-docs` may only delete names on that list plus `.erixpo/`. Never list the user's pre-existing README or source.

8. If humans will see a surface (or they just said they will build one), seed `documents/ui/` from pack templates and set `.erixpo/ui-status.md` to `draft`. Do not invent a fake brand — fill tokens in `erixpo-ui` / the first visible slice.
