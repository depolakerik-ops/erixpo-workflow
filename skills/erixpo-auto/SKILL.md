...
3. Do THE SINGLE next incomplete slice. Greenfield and constitution missing, or slice 0 scaffold still open → scaffold before product chrome (`scaffold.md` if present in pack-templates or skills). If this slice needs an unknown API or new infra: `research-scope` **narrow**, live-search official docs this year, pick official default unless USER is `ask-every-slice`. Do **not** re-research the whole stack. Typo/known pattern → skip search.
